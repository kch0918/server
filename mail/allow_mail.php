...
function allow_mail($idx){
        ...
        require_once($_SERVER['DOCUMENT_ROOT']."/include/PHPMailer-master/src/Exception.php");
            ...
            $fromName = '서버관리시스템';
            $toName = "";
            $subject = "musign care 가입승인 알림";
            $toEmail = $user_email;
            
            $message = "안녕하세요 디지털 에이전시 뮤자인입니다.<br/><br/> {$user_name} 담당자님  서버관리시스템 회원가입  승인이 완료되었음을 알려드립니다. <br/><br>";
            ...
            $message .= "<a href='https://server.musign.co.kr/'>서버관리시스템 홈페이지 바로가기</a>";
                ...
                try {
                   ...
                   /*  $mail -> addBcc("jieond@musign.net", "이지언");
                    $mail -> addBcc("chaplin@musign.net", "정기영");
                    $mail -> addBcc("rininini90@musign.net", "이수련"); */
